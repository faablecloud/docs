...
Client-side. `@faable/auth-js` runs the [Authorization Code flow with PKCE](../oauth-flows/authorization-code.md) in the browser and stores the session; no secret ships to the server. To protect server routes, API handlers, or middleware, validate the access token — see [Validate Access Tokens](../validate-access-tokens.md).
