...
| Export password hashes                   | ❌ not through the API                                                             | ✅ `firebase auth:export` (modified scrypt)                                   |
...
| A page where users manage their factors    | ✅ [`/flow/account/security`](hosted-login.md#the-one-screen-your-users-go-to-on-their-own) | You build it                          |
...
Hosting matters for more than saving work: a passkey is bound to the domain that created it, and a session cookie on one auth domain is what gives your second app single sign-on. See [Hosted Login UI](hosted-login.md).
...
`firebase auth:export` gives you users with their password hashes in Firebase's modified scrypt. Faable can't import password hashes, so:

1. Create each user over the Management API, keeping `email_verified`.
2. For Google, Apple or GitHub users, create their identity (`POST /identity`) with the provider's user id, so their next social login lands on the same account.
...
4. Replace Firebase ID-token verification in your backend with standard [access-token validation](validate-access-tokens.mdx).
