...
Do **not** use it in browsers or mobile apps — it requires a `client_secret`, which can never be shipped to a device you don't control. For user-facing apps, use the [Authorization Code flow with PKCE](authorization-code.md).
...
- **[Authorization Code Flow](authorization-code.md)** — for user-facing applications.
