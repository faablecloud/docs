...
description: Learn about the OAuth 2.0 Authorization Code flow with PKCE in Faable Auth. The most secure way for web and mobile apps to authenticate users.
---
...
| `login` | Forces the user to re-authenticate, even if there's an active session. |
...
Unknown prompt values are accepted silently — they don't change behavior. Most applications can leave `prompt` unset and rely on Faable's default behavior.
