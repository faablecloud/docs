...
| <a id="unauthorized_client"></a>`unauthorized_client`                 | 400    | The client may not use this grant. For `grant_type=password`, add `password` to the client's `grant_types`.                            |
...
| <a id="too_many_login_attempts"></a>`too_many_login_attempts`         | 429    | Too many failed sign-in attempts for this email or username from this network. Wait a few minutes and try again.                       |
