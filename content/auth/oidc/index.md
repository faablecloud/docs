...
| `prompt`                                 | `none` (silent check: `login_required` when nobody is signed in, `interaction_required` when the user still has a step to do), `login` (fresh sign-in). `select_account` shows the hosted account chooser (continue as the signed-in user, or use another account). `consent` is treated as `login`. |
| `max_age` + `auth_time`                  | Force or measure re-authentication                                                                                                                                                                                                                                                                   |
| `acr_values`                             | `urn:faable:loa:2` asks for a second factor on this login (step-up); the id token says which level was reached in `acr`.                                                                                                                                                                             |
| `login_hint`                             | Pre-fills the hosted login screen and is forwarded to social providers                                                                                                                                                                                                                               |
| `id_token_hint`                          | A previously issued id token naming the user; a session for someone else is `login_required` under `prompt=none`, a fresh sign-in otherwise                                                                                                                                                          |
| `ui_locales`                             | Forwarded to social providers                                                                                                                                                                                                                                                                        |
