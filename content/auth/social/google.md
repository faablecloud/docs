...
1. Navigate to **Auth → Settings → Custom Domains**.
1. If you haven’t configured a custom domain, your FaableAuth domain name is `{YOUR_FAABLEAUTH_NAME}.auth.faable.link`. Your redirect URI is `https://{YOUR_FAABLEAUTH_NAME}.auth.faable.link/callback`.
...
1. Choose **Google** as the connection type.
1. Enter the **Client ID** and **Client Secret** generated by Google.
