...
| Export password hashes                   | ❌ not through the API                                                       | ✅ they're in your database                                                        |
...
| Users manage their own factors                | ✅ [`/flow/account/security`](hosted-login.md#the-one-screen-your-users-go-to-on-their-own) | You build it |
...
Building your own screens gives you total control of the design. Hosting them gives you single sign-on across your apps for free (the session is a cookie on the auth domain), passkeys that work in every app (they're bound to one domain), and credentials that never touch your frontend. See [Hosted Login UI](hosted-login.md).
...
You own the `auth.users` table, so exporting is a SQL query. Faable can't import the bcrypt hashes in it, so:

1. Create each user over the Management API.
2. For social users, create their identity (`POST /identity`) from `auth.identities`, so their next Google or GitHub login lands on the same account.
3. Password users sign in with an emailed code ([passwordless](passwordless.mdx), Hobby and up) or use **Forgot password**, which works as "set your password" for an imported user.
