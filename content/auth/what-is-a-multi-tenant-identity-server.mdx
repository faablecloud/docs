...
Usually not. Most SaaS products run as **one tenant** and model customer organizations as **teams** inside it, so a user can belong to several customers and there's [one login page](hosted-login.mdx). Reserve one-tenant-per-customer for white-label or strictly regulated cases.
