...
When you run `faable deploy`, the CLI uploads your project sources and Faable's remote builder inspects them, detects the stack and builds your app. **Detection is file-based** — this page describes exactly which files the builder looks for and what it does with them, so you can shape your repo to deploy with zero configuration.
...
Your app runs on Faable's shared Node runtime image for the resolved major. If `node_modules` is missing from the uploaded sources, the builder installs dependencies automatically (`npm ci` with a lockfile, `npm install` otherwise), so the workflow needs no separate install step.
...
The platform also injects `FAABLE_HOST` (your app's public URL). `PORT` and `FAABLE_HOST` are reserved — secrets you define with those names are ignored. See [Environment & Releases](environment.mdx) for the full list of injected variables (`FAABLE_APP_ID`, `FAABLE_DEPLOY_ID`, `FAABLE_RELEASE`, …) and how the release version is resolved.
