...
To deploy one app from a monorepo (Turborepo, npm/yarn/pnpm workspaces), set the app's **Root Directory** in the dashboard — or `rootDir` in `faable.json` (the dashboard setting wins). The builder then:
...
No extra config file is needed in the repo.
