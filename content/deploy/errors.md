...
| <a id="invalid_project_description"></a>`invalid_project_description`     | 400    | The project description is not acceptable (a link, or over the length limit). `message` says why.                                                                                       |
| <a id="invalid_project_id"></a>`invalid_project_id`                       | 400    | The project in `x-faable-project` is not a valid project id (`project_` followed by 24 hex characters).                                                                                 |
...
| <a id="project_header_conflict"></a>`project_header_conflict`             | 400    | The request names two different projects (`x-faable-project` and the deprecated `x-faable-team`, or a query param). Send only `x-faable-project`.                                       |
...
| <a id="team_required"></a>`team_required`                                 | 400    | The operation needs a project: send it in the `x-faable-project` header (`project_<id>`).                                                                                               |
...
| <a id="team_or_scope_required"></a>`team_or_scope_required`               | 401    | Listing needs either a project in the `x-faable-project` header or a global scope on the token.                                                                                         |
...
| <a id="project_access_denied"></a>`project_access_denied`                 | 403    | The caller is not a member of the project named in `x-faable-project`, or the credential belongs to another project.                                                                    |
...
| <a id="auth_account_not_found"></a>`auth_account_not_found`               | 404    | There is no Faable Auth account with that id.                                                                                                                                           |
...
| <a id="tenant_token_not_issued"></a>`tenant_token_not_issued`             | 404    | Faable Auth does not issue a per-account management token for that account (the platform account).                                                                                      |
...
| <a id="repository_taken_down"></a>`repository_taken_down`                 | 409    | An app built from this repository was taken down for abuse, so the repository cannot be linked again. Not a plan limit — upgrading does not lift it. Restoring the taken-down app does. |
...
| <a id="owner_suspension_failed"></a>`owner_suspension_failed`             | 502    | The project owner could not be (un)suspended in Faable Auth. No app was touched; retry.                                                                                                 |
...
| <a id="tenant_token_unavailable"></a>`tenant_token_unavailable`           | 503    | Faable Auth could not issue a management token for the account right now. Retry later.                                                                                                  |
