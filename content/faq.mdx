...
**Faable Auth entitlements are sized by monthly active users (MAU): the number of distinct users who authenticate in a calendar month.** Each platform tier includes an MAU allowance — 50 on Free, 250 on Hobby, unlimited on Pro — and there is no per-user line on your invoice: growing past the allowance is a reason to talk to us, not an automatic charge. There are no per-feature add-ons either. See [Faable Auth pricing](auth/pricing.md).
